...
function _initImages(_aoImgs) {
  ...
  for (var i = 0, _oImg, _sSrc, _nWidth, _nHeight; i < _nLen; i++) {
    ...
    if ((_nWidth > 50 && _nHeight > 50) || (_nWidth == 0 && _nHeight == 0)) {
      addClass(_oImg, "email_img_spacer");
    }
...
function _loadImage(_aoImg, _asSrc) {
  ...
  if (_aoImg && _asSrc != _aoImg.src) {
    //不去掉的话，有边框
    removeClass(_aoImg, "email_img_spacer");
    //
...
function _showImages() {
  ...
  if (_bIsShowImg) {
    //显示背景图片
    removeClass(dom("#mailcontent"), "email_nobgimg");
    //放开webview加载图片
...
window.loadImage = function(_acId, _asPath) {
  ...
  for (var i = 0, l = imgs.length; i < l; i++) {
   //console.log("window.loadImage src = " + imgs[i].src);
    //替换掉附件内连图
    if(imgs[i].src == "cid:" + _acId) {
        //console.log("window.loadImage success");
...
window.showLoadingAll = function() {
  dom("#emailLoadAllContent").style.display = "none";
  dom("#emailLoadingAllContent").style.display = "";
}
